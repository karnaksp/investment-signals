        ...
        return _fingerprint(
            {
                ...
                "exchange_schedule_known_days": tuple(
                    item.isoformat()
                    for item in sorted(self.exchange_schedule_known_days)
                ),
...
def _opening_gap_rows(
    ...
    for ticker, candles in by_ticker.items():
        ...
        for trading_day in days:
            ...
            candidates_by_day[trading_day].append(
            ...
            history_day_is_eligible = (
                trading_day in frozenset(request.exchange_schedule_known_days)
                and trading_day not in frozenset(request.shortened_session_days)
                and (ticker, trading_day)
                not in frozenset(request.corporate_action_ticker_days)
            )
            if gap_bps is not None and history_day_is_eligible:
