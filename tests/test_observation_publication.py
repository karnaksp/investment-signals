...
def test_clickhouse_sink_maps_0351_columns_and_seals_flags(monkeypatch) -> None:
    ...
    assert query["param_session_date"] == ["2026-07-15"]
    assert query["insert_deduplication_token"] == [_observation().observation_id]
    features = json.loads(query["param_features_json"][0])
    assert features == {
        "baseline_value": 10.0,
        "detector_passed": False,
        "provenance_status": "complete",
    ...
    assert "WHERE NOT EXISTS" in request.data.decode("utf-8")
...
class _Sink:
    ...
    def persist(self, observation) -> None:
        if self.failure:
